...
export function createEncryptedPrismaAdapter(): Adapter {
  ...
  /**
   * Override linkAccount to encrypt OAuth tokens
   *
   * CRITICAL: We mutate baseAdapter directly instead of using object spread.
   * Object spread (...baseAdapter) only copies own enumerable properties and
   * LOSES prototype methods like createVerificationToken, which breaks email auth.
   */
  baseAdapter.linkAccount = async function (account: AdapterAccount) {
    ...
    // Use original linkAccount method if available
    if (originalLinkAccount) {
      // eslint-disable-next-line @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-return
      return originalLinkAccount(accountData) as Promise<AdapterAccount>;
    }
  ...
  // Return the mutated adapter with ALL methods intact
  // This preserves prototype methods like createVerificationToken (needed for email auth)
  return baseAdapter;
}
