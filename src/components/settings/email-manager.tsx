...
export function EmailManager({ userEmails }: EmailManagerProps) {
  ...
  const handleAddEmail = async () => {
    ...
    setLoading('add');
    try {
      const response = await fetch('/api/user/emails/add', {
        method: 'POST',
  ...
  const handleResendVerification = async (emailId: string) => {
    ...
    try {
      const response = await fetch(`/api/user/emails/${emailId}/resend`, {
        method: 'POST',
      });
              ...
              <div className="min-w-0 flex-1">
                ...
                {email.verifiedAt && (
                  <p className="text-xs text-muted-foreground">
                    Verified {new Date(email.verifiedAt).toLocaleDateString()}
                  </p>
                )}
              </div>
            </div>
